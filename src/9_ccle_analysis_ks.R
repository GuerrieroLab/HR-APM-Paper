...
pdf(file="CCLE_heatmap_logexp.pdf")
...
## Pathway enrichment - we'll look at the pathways enriched in intrinsic and extrinsic sigs.
load("~/Dropbox (HMS)/_projects_/B06 03 HR-APM/data/RData/msigdb_v6.1.rda")

keggs <- msigdb[grep("^KEGG_",names(msigdb))]
gos <- msigdb[grep("^GO_",names(msigdb))]
tested.gns <- readRDS("~/Dropbox (HMS)/_projects_/B06 03 HR-APM/data/RData/common_tested_gns.rds")
