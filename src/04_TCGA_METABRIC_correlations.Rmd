...
### Load data
```{r}
x <- load(file=file.path(obj_dir,"1-4_prep","gene_signature.rda")) # gene.sig
y <- load(file=file.path(obj_dir,"1-4_prep","tcga_processed_v3.rda")) # tcga.ids,tcga.clinical,tcga.syms,tcga.m1,tcga.m3
z <- load(file=file.path(obj_dir,"1-4_prep","metabric_processed_v3.rda")) # meta.ids,meta.clinical,meta.syms,meta.m1,meta.m3,meta.exp.th,meta.df,meta.sig.df
```
...
### TCGA correlation for `used.gns`
```{r}
tcga.exp <- tcga.m3[used.gns,]
tcga.sp <- list()
...
### METABRIC correlation for `used.gns`
```{r}
meta.exp <- meta.m3[used.gns,]
meta.sp <- list()
...
pdf(file.path(fig_dir,"4_tcga_metabric","corr_hallmark_tcga_ext.pdf"),width=5,height=5)
tri.heatmap(tcga.hm,main="TCGA")
dev.off()

pdf(file.path(fig_dir,"4_tcga_metabric","corr_hallmark_meta_ext.pdf"),width=5,height=5)
...
```{r}
comm.gns1 <- c(sigs$ers,sigs$immune)
...
gs1 <- sapply(gene.sig,function(gs)used.gns %in% gs)
...
tcga.sp3 <- tcga.sp1
meta.sp3 <- meta.sp1
is.hmk <- used.gns %in% hallmarks
rownames(tcga.sp3)[!is.hmk] <- colnames(tcga.sp3)[!is.hmk] <- NA
rownames(meta.sp3)[!is.hmk] <- colnames(meta.sp3)[!is.hmk] <- NA
if(1){
  pdf(file.path(fig_dir,"1_prep_genes","heatmap_corr_tcga_all_v2.pdf"),width=15,height=15)
  hc.t <- heatmap3(tcga.sp3,
            col=cols,distfun=dist,scale="none",
            balanceColor=TRUE,
            ColSideColors=lcols[,1:3],
            RowSideColors=lcols[,1:3],
            margins=c(9,9),
            cexRow=3,cexCol=3,
            main="TCGA (778 genes)")
  dev.off()
  
  pdf(file.path(fig_dir,"1_prep_genes","heatmap_corr_meta_all_v2.pdf"),width=15,height=15)
...
pdf(file.path(fig_dir,"4_tcga_metabric","heatmap_mean_tcga_mods_v2.pdf"),width=7,height=7)
...
tcga.sp1 <- tcga.sp$hr[comm.gns,comm.gns]
meta.sp1 <- meta.sp$hr[comm.gns,comm.gns]
...
sig.lcols <- cbind(hallmark=lcol1,module=lcol3,subclass=lcol4)

if(0){
  saveRDS(sig.lcols,file=file.path(obj_dir,"1-4_prep","sigs_sidecols.rds"))
}

is.hmk <- comm.gns %in% hallmarks
rownames(tcga.sp1)[!is.hmk] <- colnames(tcga.sp1)[!is.hmk] <- NA
rownames(meta.sp1)[!is.hmk] <- colnames(meta.sp1)[!is.hmk] <- NA

##
rng <- range(tcga.sp1)#[comm.gns,comm.gns])
...
pdf(file.path(fig_dir,"4_tcga_metabric","heatmap_corr_tcga_sub_v2.pdf"),width=15,height=15)
a <- heatmap3(tcga.sp1,
              Colv=d.tcga,Rowv=d.tcga,
              col=cols,scale="none",
              ColSideColors=sig.lcols[comm.gns,],
              RowSideColors=sig.lcols[comm.gns,],
              margins=c(9,9),
              cexRow=3,cexCol=3,              
              balanceColor=TRUE)
dev.off()
...
pdf(file.path(fig_dir,"4_tcga_metabric","heatmap_corr_meta_sub_v2.pdf"),width=15,height=15)
b <- heatmap3(meta.sp1,
...
```{r}
saveRDS(sigs, file=file.path(obj_dir,"1-4_prep","modules3_ers_immune_cc.rds")) # ers, immune, cc
saveRDS(mod.gns, file=file.path(obj_dir,"1-4_prep","module_genes_final_122923.rds")) # module genes
