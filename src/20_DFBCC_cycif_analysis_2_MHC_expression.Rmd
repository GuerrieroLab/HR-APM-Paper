...
### Re-install CycifAnalyzeR after updating the package
```{r reinstall_CyCIF_ library,eval=F}
 suppressPackageStartupMessages({
  devtools::install_github("kenichi-shimada/CycifAnalyzeR",force=TRUE)
...
if(0){
  ...
  for(nm in nms){
    cat(nm,"..\n")
    x1 <- cs1[[nm]]
    obj <- defineTumorStroma(x1, n.cores = 7, dth=dth, concavity = 0.8, plot = FALSE)
    saveRDS(obj,file.path(cycif_obj_dir,"distance",paste0("defTS_",nm,".rds")))
  }
}else{
  lst.dists  <- list()
  for(nm in nms){
    ...
    # obj <- defineTumorStroma(x1, n.cores = 7, dth=dth, concavity = 0.8, plot = FALSE)
    # saveRDS(obj,file.path(cycif_obj_dir,"distance",paste0("defTS_",nm,".rds")))
    lst.dists[[nm]] <- readRDS(file.path(cycif_obj_dir,"distance",paste0("defTS_",nm,".rds")))
  }
}
```
...
if(0){
  for(nm in nms){
    ...
    obj <- computeCN(x,type="frnn",
                     ...
                     seed=123)
    saveRDS(obj,file.path(cycif_obj_dir,"cn_frnn",paste0("cn_frnn_",nm,".rds")))
  }
}else{
  lst.frnn <- list()
  for(nm in nms){
    lst.frnn[[nm]] <- readRDS(file.path(cycif_obj_dir,"cn_frnn",paste0("cn_frnn_",nm,".rds")))
  }
}
```
...
### *****k-means clustering*****
```{r}
# [[[check these dim/length here]]]
## distance from tumor border
cts <- cell_types(cs1)
...
```{r}
if(1){
  pdf(file.path(cycif_fig_dir,"cn","n_cells_dist.pdf"),width=7,height=7)
  par(mar=c(5,5,3,3))
...
if(0){
  # compute distance betwen pca_data and centers -> straightforward but slow
  cluster_assignments <- apply(pca_data_all,1,function(x){
    dists <- apply(km$centers,1,function(y)sum((x-y)^2))
    return(which.min(dists))
  })
}else{
  # faster calculation of extrapolating kmeans
  ...
  cluster_assignments0 <- cluster_assignments <- 
    max.col(-squared_dists, ties.method = "first")

  freq.clusts <- table(cluster_assignments)/length(cluster_assignments)
}

# sorted.clusts <- names(sort(table(cluster_assignments),decreasing=T))
# cluster_assignments1 <- as.numeric(factor(cluster_assignments,levels=as.integer(sorted.clusts)))
```
...
```{r}
if(0){
  saveRDS(cluster_assignments,
          file.path(cycif_obj_dir,"cluster_assignments_kmeans_all_v1_102624.rds"))
}else{
  cluster_assignments <- readRDS(file.path(cycif_obj_dir,"cluster_assignments_kmeans_all_v1_102624.rds"))
}
```
...
### HLAABC vs HLADPB1
pdf(file.path(cycif_fig_dir,"cn_exp","scatterplot_HLAABC_HLADPB1_v2.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
...
##
pdf(file.path(cycif_fig_dir,"cn_exp","boxplot_HLAABC_nTcells_v2.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
...
##
pdf(file.path(cycif_fig_dir,"cn_exp","boxplot_HLADPB1_nTcells_v2.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
...
pdf(file.path(cycif_fig_dir,"cn_exp","scatterplot_HLAABC_HLADPB1_lm_v2.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
smoothScatter(df.cn.ts$HLAABC,df.cn.ts$HLADPB1,
...
pdf(file.path(cycif_fig_dir,"cn_exp","volcano_cts_in_HLAABC_HLADPB1.pdf"),width=7,height=7)
print(p)
dev.off()
```
...
```{r}
if(0){
  saveRDS(df.cn,file.path(cycif_obj_dir,"df.cn.rds"))
}else{
  df.cn <- readRDS(file.path(cycif_obj_dir,"df.cn.rds"))
}
```
...
```{r}
cltab <- table(df.cn$smpl,df.cn$cluster)
...
pdf(file.path(cycif_fig_dir,"cn","clust_rel_normalized_sp.pdf"),width=12,height=7)
...
pdf(file.path(cycif_fig_dir,"cn","spearman_cor_clust_freq_vs_ers.pdf"),width=7,height=7)
...
pdf(file.path(cycif_fig_dir,"cn","heatmap3_ctfreq_v2.pdf"),width=7,height=7)
...
```{r}
med.dist <- (df.cn %>% group_by(cluster) %>% summarize(median=median(d,na.rm=T)))
...
pdf(file.path(cycif_fig_dir,"cn","boxplot_cluster_distance_v2.pdf"),width=7,height=7)
print(p)
dev.off()
```
...
### save `lst.cst.def` for further analysis
```{r}
if(0){
  saveRDS(lst.cst.def,file.path(cycif_obj_dir,"lst.cst.def.rds"))
}else{
  lst.cst.def <- readRDS(file.path(cycif_obj_dir,"lst.cst.def.rds"))
}
```
...
for(ab in used.abs){
  ...
  pdf(file.path(cycif_fig_dir,"per_smpl",paste0("mean_exp_smpl_",ab,"_v2.pdf")),width=7,height=7)
...
pdf(file.path(cycif_fig_dir,"per_smpl","cor_sp_mean_exp_smpl.pdf"),width=7,height=7)
...
if(0){
  ...
  pdf(file.path(cycif_fig_dir,"regression","r2_models_v3.pdf"),width=2.5,height=7)
  print(p)
  dev.off()
  ...
  save(AICs,BICs,file=file.path(cycif_obj_dir,"model_selection_exp_cluster_v3.rda"))
  aic.bic <- round(cbind(aics,bics))
  colnames(aic.bic) <- paste0(rep(c("AIC","BIC"),each=3),".",rep(c("s","c","sc"),2))
  write.csv(aic.bic,file=file.path(cycif_obj_dir,"model_selection_exp_aic_bic_v3.csv"))
}else{
  load(file=file.path(cycif_obj_dir,"model_selection_exp_cluster_v3.rda")) # AICs,BICs,lmers
}
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","ERa expression per distance_v2.pdf"),width=5,height=5)
print(p.er1)
dev.off()
```
...
p.er2 <- ggplot(e300,aes(x=nT,y=expression)) +
  ...
  theme_minimal() +
  ggtitle("ERa expression in tumor core and stroma\n(100-200um from border)") +
  xlab("# of T cells per CN") +
  ylab("ERa Expression") + 
  theme(legend.position="none")

pdf(file.path(cycif_fig_dir,"cn_exp","ERa vs nT.pdf"),width=5,height=5)
print(p.er2)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","HLAABC expression per distance_v2.pdf"),width=5,height=5)
print(p.hla1)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","HLAABC vs nT.pdf"),width=5,height=5)
print(p.hla2)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","HLADPB1 expression per distance_v2.pdf"),width=5,height=5)
print(p.dpb1)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","HLADPB1 vs nT.pdf"),width=5,height=5)
print(p.hla2)
dev.off()
```
...
if(0){
  ...
  for(nm in nms){
    cat(nm,"\n")
    tmp <- df.cn %>%
      mutate(is_ge10 = is.ge10) %>%
      filter(smpl == nm) %>%
      # filter(is_ge10) %>% #is.selected
  ...
  dt <- rbindlist(dts) %>%
    as.data.frame() %>%
    filter(is_ge10)
  saveRDS(dt,file=file.path(cycif_obj_dir,"df.cn_exp_per_ct.RData"))
}else{
  dt <- readRDS(dt,file=file.path(cycif_obj_dir,"df.cn_exp_per_ct.RData"))
}
...
pdf(file.path(cycif_fig_dir,"cn_exp","exp_HLAABC_ER_dependence_v2.pdf"),width=5,height=7)
print(p.hla1.dep)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","exp_HLAABC_ER_independence_v2.pdf"),width=5,height=7)
print(p.hla1.indep)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","exp_HLADPB1_ER_dependence_v2.pdf"),width=5,height=7)
print(p.hla2.dep)
dev.off()
```
...
##
pdf(file.path(cycif_fig_dir,"cn_exp","barplot_HLAABC_HLADPB1_ER_dependence_v2.pdf"),width=6,height=7)
par(mar=c(12,5,4,1))
...
### `dt` - Mean expression for _each protein_ per _cell types_
...
if(0){
  saveRDS(lmers2,file=file.path(cycif_obj_dir,"lmer_protein_exp_per_cell_type_v3.rds"))
}else{
  lmers2 <- readRDS(file=file.path(cycif_obj_dir,"lmer_protein_exp_per_cell_type_v3.rds"))
}
```
...
if(0){
  saveRDS(coefs2,file=file.path(cycif_obj_dir,"protein_exp_mixed_effect_per_cell_type_v3.rds"))
}else{
  coefs2 <- readRDS(file=file.path(cycif_obj_dir,"protein_exp_mixed_effect_per_cell_type_v3.rds"))
}
```
...
pdf(file.path(cycif_fig_dir,"cn_exp","cor_plot_mixed-effect","mixed_effect_ERa_per_ct_v4.pdf"),
    width=7,height=7)
...
pdf(file.path(cycif_fig_dir,"cn_exp","cor_plot_mixed-effect","mixed_effect_HLAABC_per_ct_v4.pdf"),
    width=7,height=7)
...
pdf(file.path(cycif_fig_dir,"cn_exp","cor_plot_mixed-effect","mixed_effect_HLADPB1_per_ct_v4.pdf"),
    width=7,height=7)
...
pdf(file.path(cycif_fig_dir,"cn_exp","cor_plot_mixed-effect","mixed_effect_PD1_per_ct_v4.pdf"),
    width=7,height=7)
h3(t(coefs2.1[,rev(17:20)]),Colv=NA,Rowv=NA,scale="row",cexRow=.5,col=cols,
...
fn <- paste0("co-occurence_spearman_w_proteins_per_ct_v3.pdf")

pdf(file.path(cycif_fig_dir,"cn_exp","cor_plot_mixed-effect",fn),width=7,height=7)
...
p.val <- t.test(hla ~ is.erdep,alternative="greater")$p.value
...
p.erd <- ggplot(df.cn.hla.erd,aes(x=class,y=SCC)) +
  ...
  theme_minimal() +
  ggtitle("SCC between HLA and ERa expression") +
  ylab("SCC with ERa expression in cancer cells") +
  xlab("")

pdf(file.path(cycif_fig_dir,"cn_exp","cor_plot_mixed-effect","cor_hla_erd_v3.pdf"),width=4,height=5)
print(p.erd)
dev.off()
```

## X. lm coefs - trend for per cell type expression
### among samples
```{r}
used.abs <- c("ERa","HLAABC","HLADPB1","PD1")
df.cn5 <- df.cn %>%
...
saveRDS(lm.coefs,file=file.path(cycif_obj_dir,"lm_coefs_smpls.rds"))
write.csv(lm.coefs,file=file.path(cycif_fig_dir,"cn_various","cor_plot_mixed-effect","lm_coefs_smpls.csv"))
```
...
pdf(file.path(cycif_fig_dir,"per_smpl","violin_HLAABC_per_ct.pdf"),width=7,height=7)
print(p)
dev.off()
...
pdf(file.path(cycif_fig_dir,"per_smpl","pval_HLAABC_per_ct.pdf"),width=7,height=7)
par(mar=c(10,10,4,1))
...
pdf(file.path(cycif_fig_dir,"per_smpl","violin_HLADPB1_per_ct.pdf"),width=2.5,height=7)
print(p)
dev.off()
```
