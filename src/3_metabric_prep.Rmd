...
# Assuming 'mm1' is your matrix of gene expression data, 'uniq.syms1' contains unique gene symbols,
# and 'exp.th' is your expression threshold.

calc_wcss <- function(data) {
  km1 <- kmeans(data, centers = 1, nstart = 25)
  km2 <- kmeans(data, centers = 2, nstart = 25)
  km <- list(km1,km2)
  return(sapply(km, function(x) x$tot.withinss))
}

# Function to calculate simplified BIC-like score for k=1 and k=2
calc_bic_like <- function(wcss, n, p) {
  penalty <- log(n) * p
  bic_like <- log(wcss) + penalty
  return(bic_like)
}

# Parallel processing to evaluate each gene
lst <- parallel::mclapply(uniq.syms1, function(sym, data=mm1, exp.th) {
  this <- t(data[sym, , drop = FALSE])  # Ensure it's a matrix, even for a single row
  n <- nrow(this)
  
  # Calculate WCSS for k=1 and k=2
  wcss <- calc_wcss(this)
  
  # Number of parameters: k * number of dimensions - 1
  p <- c(1, 2) * ncol(this) - 1
  
  # Calculate simplified BIC-like score
  bic_like <- mapply(calc_bic_like, wcss, n, p)
  
  # Choose model with lower BIC-like score
  g <- which.min(bic_like)
  
  # Additional calculations based on chosen g
  if(g == 1) {
    peaks <- NA
    is.between <- NA
  } else {
    # Assume kmeans result for k=2 for peak identification
    km2 <- kmeans(this, centers = 2, nstart = 25)
    peaks <- sort(km2$centers)
    is.between <- NA  # Define based on your criteria for 'is.between'
  }
  
  is.bg <- min(peaks, na.rm = TRUE) < exp.th
  max.sig <- max(peaks, na.rm = TRUE) > exp.th
  ...
  return(c(sym = sym,
           ...
           sig = max.sig,
           p1 = peaks[1],
           p2 = ifelse(length(peaks) > 1, peaks[2], NA),
           is.between = is.between
  ))
}, mc.cores = detectCores() - 1, data = mm1, exp.th = exp.th)

# Convert list to a data frame or another preferred structure for further analysis
...
##
 lst1 <- as.data.frame(do.call(rbind,lst)) %>%
  ...
  mutate(is.between=as.logical(is.between)) %>%    
