...
mod.side.cols <- sig.lcols[mod.gns,]
```

```{r}
save(exp.ct.cols1,st.uniq.cols,mod.cols,mod.side.cols,file=file.path(obj_dir1,"color_schema.rda"))
```
...
if(0){
  ...
  saveRDS(exp.gn.by.ct,file.path(obj_dir1,"mod_exp_by_ct_v4.rds"))
}else{
  exp.gn.by.ct <-readRDS(file.path(obj_dir1,"mod_exp_by_ct_v4.rds"))  
}
...
if(0){
  ...
  saveRDS(exp.gn.by.smpl,file.path(obj_dir1,"mod_exp_by_smpl_v4.rds"))
}else{
  exp.gn.by.smpl <-readRDS(file.path(obj_dir1,"mod_exp_by_smpl_v4.rds"))  
}
```
...
#### [Fig S6X - not used] module gene exprssion in cancer cells
```{r}
...
if(0){
  save(df.pts,df.ers,file=file.path(obj_dir1,"mod_activity_visvader_all_102324.rda"))
}else{
  load(file=file.path(obj_dir1,"mod_activity_visvader_all_102324.rda")) # df.pts, df.ers
}
```
...
## save objects
```{r}
save(mods,mod.gns,df.tum.exp,file=file.path(obj_dir1,"df_gn_exp_tum_102324.rda"))
```
