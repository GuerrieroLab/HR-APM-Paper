...
### Load Data
```{r}
mdacc_metadata <- read.csv(file.path(data_dir, "clinical", "mdacc", "MDACC 74 tumors description.csv")) %>%
  mutate(Subtype.classification = factor(c("HR+", "DP", "TNBC", "HER2+")[((HR.status == "negative") * 2 + (HER2.status == "positive") + 1)], 
                                         levels = c("HR+/HER2-", "HR+/HER2+", "HR-/HER2-", "HR-/HER2+")))
names(mdacc_metadata)[1] <- "sample_name"

mdacc_data <- read.csv(file.path(data_dir, "clinical", "mdacc", "MDACC 74 tumors log2 tpm RNA-seq data.csv"))
names(mdacc_data)[1] <- "hgnc_symbol"

sigs <- readRDS(file = file.path(obj_dir, "1-4_prep", "module_genes_final_122923.rds"))
comm.gns <- unlist(sigs)
comm.ers <- sigs$ERS
comm.apm.tc <- unlist(sigs[-1])
...
for(cl in smpl.class){
  pdf(file.path(fig_dir,"7_mdacc_subtypes",paste0("mdacc_",cl,"_v4.pdf")),
      width=7,height=7)	
	par(cex.main=3)
...
for (cl in smpl.class){
  pdf(file.path(fig_dir, "7_mdacc_subtypes", paste0("mean_mdacc_", cl, "_v4.pdf")), width = 7, height = 7)
  ...
  dev.off()
  write.csv(mean.mdacc.pe[[cl]], file.path(fig_dir, "7_mdacc_subtypes", paste0("mean_mdacc_", cl, "_v4.csv")))
}
```
...
esr1.mdacc$esr1 <- as.numeric(esr1.mdacc$esr1)

p <- ggplot(esr1.mdacc, aes(Subtype.classification, esr1)) +
  geom_violin(fill = 'grey80') +
  geom_jitter(height = 0, width = 0.2, size = 0.5) +
  labs(title = "MDACC", x = "", y = "ESR1 expression (log2(TPM+1)")
