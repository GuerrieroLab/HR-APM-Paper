...
source(file.path(src_dir,"utils","subtype_violin_plot.R"))
```
...
```{r}
p <- subtype_violin_between(dfx=dfx.tcga,mod="IMMUNE") + ggtitle("ERS vs Pan-immune (TCGA)")
pdf(file.path(fig_dir,"06_TCGA_METABRIC_subtypes","violin","tcga_ers_imm_v2.pdf"),width=3,height=6)
print(p)
dev.off()
...
```{r}
p <- subtype_violin_between(dfx=dfx.meta,mod="IMMUNE") + ggtitle("ERS vs Pan-immune (METABRIC)")
pdf(file.path(fig_dir,"06_TCGA_METABRIC_subtypes","violin","meta_ers_imm_v2.pdf"),width=3,height=6)
print(p)
dev.off()
