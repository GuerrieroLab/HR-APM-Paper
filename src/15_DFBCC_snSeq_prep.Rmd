...
```{r}
cts <- seurat_harmony$celltype_broad_v3
cts[cts=="Other"] <- NA
cts <- factor(cts)
seurat_all_v1 <- subset(seurat_harmony,cells=names(cts[!is.na(cts)]))

png(file.path(fig_dir1, "umap_wp_celltype_wo_other.png"), width = 700,height = 700)
p <- DimPlot(seurat_all_v1, group.by = "celltype_broad_v3", label = FALSE, reduction = "umap", raster=FALSE, pt.size=0.1) +
...
### [Fig S7B] DotPlot
```{r}
seurat_all_v1@meta.data$celltype_broad_v3_rev <- factor(seurat_all_v1@meta.data$celltype_broad_v3,levels=rev(levels(cts)))

pdf(file.path(fig_dir1,"dotplot_wp_celltype.pdf"),width=12,height=6)
DotPlot(seurat_all_v1, features = unique(mks), group.by = "celltype_broad_v3_rev") +
...
df.gs <- readRDS(file.path(obj_dir,"11_dfbcc_txn","df_pcs_mod_activity_v3.rds")) %>%
  ...
  arrange(desc(ERS)) %>%
  filter(bot %in% unique(seurat_harmony$sample))
bots.used <- df.gs$bot
new.ids <- df.gs$new
...
##
load(file.path(obj_dir,"15_dfbcc_snseq","exp.ct.cols2.rda"))
set.seed(123)
tab <- table(seurat_all_v1$celltype_broad_v3,seurat_all_v1$sample)[-13,bots.used]
...
```{r}
saveRDS(nfreq,file.path(obj_dir1,"nstat_dfbcc_sn.rds"))
```
...
cors <- apply(nfreq[1:12,],1,function(x)cor(x,df.gs.1[2:5]))
rownames(cors) <- nms

cor.pvals <- apply(nfreq[1:12,],1,function(x){
  sapply(df.gs.1[2:5],function(y){
    tmp <- cor.test(x,y)$p.value
  })
})
# heatmap3(cors,balanceColor=T,scale="none",Colv=NA,Rowv=NA,margins=c(10,5))
...
saveRDS(seurat_harmony, file=file.path(obj_dir1, "seurat_harmony_v2.rds")) 
