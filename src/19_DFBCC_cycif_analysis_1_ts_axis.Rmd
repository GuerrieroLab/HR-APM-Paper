...
for(nm in nms){
  ...
  if(0){
    ## distance of cancer cells
    xx <- hist(tum.d,breaks=1000,main="distance of cancer cells from border",plot=F)
    ymax <- max(xx$counts)
  
    fn <- paste0("dist_tum_",nm,".png")
    pdf(file.path(cycif_fig_dir,"slide_dist","dist_tumor",fn),width=7,height=7)
    xx <- hist(tum.d,breaks=1000,main="distance of cancer cells from border",ylim=c(0,ymax * .1))
    abline(v=c(0,304,1000),col=2)
    dev.off()
  
    hist(tum.pos.d,breaks=1000)
  }
...
```{r}
if(0){
  save(freq.ts,sds,file=file.path(cycif_obj_dir,"freq_ts.rda"))
}else{
  load(file=file.path(cycif_obj_dir,"freq_ts.rda"))
}
```
...
pdf(file.path(cycif_fig_dir,"ts_axis","freq_cancer_buds.pdf"),width=7,height=7)
par(mar=c(5,4,4,10))
...
pdf(file.path(cycif_fig_dir,"ts_axis","ERS_vs_tumor_buds.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
...
pdf(file.path(cycif_fig_dir,"ts_axis",fn),width=6,height=7)
print(p)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"ts_axis",fn),width=3,height=7)
print(p)
dev.off()
...
pdf(file.path(cycif_fig_dir,"ts_axis","cor_w_ts_ratio","pearson_imm.pdf"),width=8,height=7)
heatmap3(pe[,-1],Colv=NA,Rowv=NA,scale="none",balanceColor=TRUE,margins=c(10,10),main="pearson")
dev.off()

pdf(file.path(cycif_fig_dir,"ts_axis","cor_w_ts_ratio","spearman_imm.pdf"),width=8,height=7)
...
for(ct in ucts){
  for(m in mod.nms[-1]){
    ...
    pdf(file.path(cycif_fig_dir,"ts_axis","cor_w_ts_ratio",
...
for(nm in nms){
  ...
  png(file.path(cycif_fig_dir,"ts_axis","slide_dist",fn),width=700,height=700)
...
for(nm in nms){
  ...
  png(file.path(cycif_fig_dir,"ts_axis","slide_ncells",fn),width=700,height=700)
...
#### `moving_average` function: smoothening the counts
```{r}
...
pdf(file.path(cycif_fig_dir,"t_cell","T_freq_per_distance_v2.pdf"),width=7,height=7)
print(p1)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"t_cell","T_density_per_CN_per_distance_v2.pdf"),width=7,height=7)
print(p2)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"t_cell","PD1_per_distance_v2.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
...
if(0){
  ...
  saveRDS(areas,file.path(cycif_obj_dir,"cs1_areas.rds"))
}else{
  areas <- readRDS(file.path(cycif_obj_dir,"cs1_areas.rds"))
}
...
```{r}
##
pdf(file.path(cycif_fig_dir,"t_cell","T_density_vs_PD1_v2.pdf"),width=7,height=7)
print(p1)
dev.off()

##
pdf(file.path(cycif_fig_dir,"t_cell","clustered_T_vs_PD1_v2.pdf"),width=7,height=7)
print(p2)
dev.off()

##
pdf(file.path(cycif_fig_dir,"t_cell","clustered_T_density_vs_PD1_v2.pdf"),width=7,height=7)
print(p3)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"cn","cor_sp_freq_v2.pdf"),width=7,height=7)
...
pdf(file.path(cycif_fig_dir,"ts_axis","macs","plot_mac_freq_v2.pdf"),width=7,height=7)
print(p.m)
dev.off()
```
...
pdf(file.path(cycif_fig_dir,"ts_axis","endo","median_endo_size_freq_v2.pdf"),width=7,height=7)
print(p)
dev.off()
```
...
```{r}
# e.df.sum <- data.frame(d=ed1,size=es1)  
# e.df1 <- e.df.sum %>%  filter(d < 1000 & d > -500 & size < 200) 
# 
# abline(h=0,v=0)
# lines(e.df.r,lty=2)
# smpl <- rep(nms,sapply(e.size,length))
```

```{r}
# n.endo <- sapply(e.size,length)
# hist(es1,breaks=1000,ylim=c(0,1000),xlim=c(0,200))
# mean.es1 <- tapply(es1>20,smpl,mean)
# plot(n.endo,mean.es1)
```

```{r}
if(0){
  pdf(file.path(cycif_fig_dir,"cn_various","endo_size_dist.pdf"),width=7,height=7)
  par(mar=c(5,5,3,3))
...
n.endo <- data.frame(smpl=factor(smpl,levels=nms),e.size=es1,e.dist=ed1) %>%
  mutate(d1=bin_values(e.dist,width=20))
  # group_by(d1) %>%
  # summarize(n.all=sum(n.cells),.groups="drop")
...
pdf(file.path(cycif_fig_dir,"ts_axis","endo","plot_endo_size_freq_v2.pdf"),width=7,height=7)
print(p)
dev.off()
```
...
```{r}
# cors <- c(cor500[c(1,3)],cor0)[c(1,3:4,2,5:7)]
...
pdf(file.path(cycif_fig_dir,"ts_axis","endo","endo_size_loc and NFkB_v2.pdf"),width=5,height=7)
print(p)
dev.off()
```
