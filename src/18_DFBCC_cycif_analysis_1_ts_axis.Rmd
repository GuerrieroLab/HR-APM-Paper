...
if(!dir.exists(file.path(fig_dir1,"distance"))){dir.create(file.path(fig_dir1,"distance"))}
if(!dir.exists(file.path(fig_dir1,"cn_frnn"))){dir.create(file.path(fig_dir1,"cn_frnn"))}
```
...
# select sn0(=1000) cels per each T cell density: ids
sn0 <- 1e3
set.seed(12345)
...
rsqs <- sapply(lms,function(lm1)summary(lm1)$adj.r.squared)
legs <- paste0(c("CD8T","CD4T_nonTreg","CD4T_Treg","T_other")," (R2=",signif(rsqs,2),",p=",signif(p.vals,2),")")
...
pdf(file.path(fig_dir1,"t_cell","PD1_vs_density.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
...
if(0){
  ...
  areas <- readRDS(file.path(obj_dir1,"sample_areas.rds"))
}

```
## [Fig] Area within tumor core
```{r}
if(0){
  ## compute T cell density per sample
  nms <- names(cs1)
  ...
  for(nm in nms){
    cat(nm,"\n")
    core.areas[[nm]] <- computeArea(cs1[[nm]],dth=dth,plot=FALSE) 
  }
  core.areas <- do.call(c,core.areas)

  saveRDS(core.areas,file.path(obj_dir1,"sample_core_areas.rds"))
}else{
  core.areas <- readRDS(file.path(obj_dir1,"sample_core_areas.rds"))
}
