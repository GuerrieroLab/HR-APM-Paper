...
### [used?] stratify with metadata
```{r}
clin <- readRDS(file.path(obj_dir,"clinical","metadata.rds"))
```


## [Fig S7C] linear regression
```{r}
coef.pts.2 <- coef.pts.1 %>% 
  tibble::rownames_to_column("new") %>%
  mutate(idx=as.numeric(sub("cy","",new))) %>%
  arrange(idx) %>%
  mutate(new = factor(new,levels=new)) %>%
  dplyr::select(-idx) %>%
  dplyr::select(new,ERS) %>%
  dplyr::rename(sn_ERS = ERS)

df.gs.2 <- as.data.frame(df.gs) %>% 
  filter(new %in% coef.pts.2$new) %>%
  dplyr::select(new,ERS) %>%
  dplyr::rename(bulk_ERS = ERS)

ct.freq <- as.data.frame(t(nstat)) %>%
  tibble::rownames_to_column("new") %>%
  mutate(new = factor(new,levels=new)) %>% 
  dplyr::select(new,Tumor)

df.ers <- inner_join(coef.pts.2,df.gs.2,by="new") %>%
  inner_join(ct.freq,by="new") %>%
  mutate(sim_bulk = sn_ERS * Tumor)

lm0 <- lm(bulk_ERS ~ sn_ERS + Tumor, data=df.ers)
lm1 <- lm(bulk_ERS ~ sn_ERS, data=df.ers)
lm2 <- lm(bulk_ERS ~ Tumor, data=df.ers)

r.sq <- c(sn_freq=summary(lm0)$r.squared,
  sn=summary(lm1)$r.squared,
  freq=summary(lm2)$r.squared)

if(1){
  pdf(file.path(fig_dir,"v2","mod_activity",paste0("barplot_rsq_ERS_h_042624.pdf")),
      width=7,height=3)
  barplot(rev(r.sq),las=2,xlim=c(0,1),col=brewer.pal(3,"Set1"),
          main="R-squared of linear regression model",
          xlab="Model",ylab="R-squared",horiz=T)
  dev.off()
}

if(1){
  pdf(file.path(fig_dir,"v2","mod_activity",paste0("plot_ERS_activity_comparison_042624.pdf")),
      width=7,height=7)
  par(mar=c(5,5,3,3))
...
msigdb.1 <- c(msigdb,mods,cons.gns[2:3])
prefix <- sub("_.+","",names(msigdb))
table(prefix)

all.syms <- unique(unlist(msigdb.1))
txn.syms <- rownames(ldf.2)
common.syms <- intersect(all.syms,txn.syms) # 16207

identical(colnames(ldf.2),df.gs %>% arrange(bot) %>% pull(bot))
colnames(ldf.2) <- df.gs %>% arrange(bot) %>% pull(new)
uniq.pts <- df.gs$new
ldf.2.1 <- ldf.2[common.syms,uniq.pts]
```


```{r}
cycif.nsh <- readRDS("/Users/kenichishimada/Dropbox (HMS)/_projects_/B06 03 HR-APM/cycif/robj/nstat_cycif_042624.rds")

uniq.pts <- intersect(colnames(ldf.2.1),colnames(cycif.nsh))
ldf.2.1 <- ldf.2.1[,uniq.pts]
tum.freq <- cycif.nsh["Tumor",uniq.pts]
cors <- as.vector((cor(t(ldf.2.1),tum.freq,method="spearman")))
names(cors) <- rownames(ldf.2.1)

head(sort(cors,decreasing=T),100)
head(sort(cors,decreasing=T),100)
sorted.cors <- sort(cors,decreasing=F)

fg1 <- fgsea(pathways = msigdb.1, 
                  stats    = sorted.cors,
                  minSize  = 15,
                  maxSize  = 500) %>%
  mutate(slp = -log10(padj) * sign(ES))

fg2 <- fg1 %>% mutate(text = "") %>% mutate(lab=1)
fg2$text[match(c(names(mods),names(cons.gns[2:3])),fg2$pathway)] <- 
  c(names(mods),"TNFa/NFkB_int","IFN-I_int")

### violin plot of the FGSEA results highlighting slp and the custom modules
p <- ggplot(fg2,aes(x=lab,y=slp)) + 
  geom_violin() +
  geom_point(data=fg2 %>% filter(pathway %in% names(mods)),aes(y=slp),col="red",pch=95,size=5) +
  geom_text_repel(data=fg2 %>% filter(pathway %in% names(mods)),aes(label=text),size=3) +
  # theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  # scale_fill_manual(values=rep("gray",nrow(fg1))) +
  theme_minimal() +
  theme(legend.position="none")

pdf(file.path(fig_dir,"11_dfbcc_txn","fgsea_violin_bulk_042624.pdf"),width=4,height=7)
print(p)
dev.off()

fg1 %>% arrange(desc(slp)) %>% head(20)
fg1 %>% arrange((slp)) %>% head(20)
# save(cors.ers,cors.tu,fg.ers,fg.tu,file=file.path(obj_dir,"fgsea_dfci_sn_042624.rda"))
```
```{r}
saveRDS(nstat,file.path(obj_dir,"nstat_dfci_sn.rds"))
```