...
### Set path
```{r}
fig_dir1 <- file.path(fig_dir,"8_acosog_cohorts","v2")
```
...
#### module genes (`mods`)
```{r}
mods <- readRDS(file=file.path(obj_dir,"1-4_prep","module_genes_final_122923.rds"))
```
...
pdf(file=file.path(fig_dir,"8_acosog_cohorts","ACSOG_scatter_baseline.pdf"),width=7,height=7)
print(p)
dev.off()
```
...
p <- ggplot(ers,aes(x=Pre,y=diff)) +
  ...
  ggtitle(paste0("ERS (SCC:",sp,")")) +
  xlab("ERS exprs. at Pre-tx") +
  ylab("d.ERS") +
  theme_minimal()

pdf(file=file.path(fig_dir1,"ERS_pre_post.pdf"),width=7,height=7)
print(p)
dev.off()
...
pval <- t.test(pd.need2 %>% filter(time_point == "Pre") %>% pull(ERS),
       ...
       paired=T)$p.value
tmp <- data.frame(x=1.5,y=max(pd.need2$ERS),label=paste0("p=",signif(pval,3)))
p <- ggplot(pd.need2,aes(x=time_point,y=ERS)) +
  ...
  ggtitle("ERS") +
  theme_minimal()
pdf(file=file.path(fig_dir1,"ERS_pre_post.pdf"),width=4,height=7)
print(p)
dev.off()
...
pval <- t.test(pd.need2 %>% filter(time_point == "Pre") %>% pull(`TNFa/NFkB`),
       ...
       paired=T)$p.value
tmp <- data.frame(x=1.5,y=max(pd.need2$`TNFa/NFkB`),label=paste0("p=",signif(pval,3)))
p <- ggplot(pd.need2,aes(x=time_point,y=`TNFa/NFkB`)) +
  ...
  ggtitle("TNFa/NFkB") +
  theme_minimal()
pdf(file=file.path(fig_dir1,"NFkB_pre_post.pdf"),width=4,height=7)
print(p)
dev.off()
...
pval <- t.test(pd.need2 %>% filter(time_point == "Pre") %>% pull(`APM/TC`),
       ...
       paired=T)$p.value
tmp <- data.frame(x=1.5,y=max(pd.need2$`APM/TC`),label=paste0("p=",signif(pval,3)))
p <- ggplot(pd.need2,aes(x=time_point,y=`APM/TC`)) +
  ...
  ggtitle("APM/TC") +
  theme_minimal()
pdf(file=file.path(fig_dir1,"APMTC_pre_post.pdf"),width=4,height=7)
print(p)
dev.off()
...
pval <- t.test(pd.need2 %>% filter(time_point == "Pre") %>% pull(`IFN-I`),
       ...
       paired=T)$p.value
tmp <- data.frame(x=1.5,y=max(pd.need2$`IFN-I`),label=paste0("p=",signif(pval,3)))
...
p <- ggplot(pd.need2,aes(x=time_point,y=`IFN-I`)) +
  ...
  ggtitle("IFN-I") +
  theme_minimal()
pdf(file=file.path(fig_dir1,"IFN_pre_post.pdf"),width=4,height=7)
print(p)
dev.off()
```
...
if(0){
  ggplot(pd.need1,aes(x=PC1,y=PC2,color=ERS,shape=time_point)) + 
    geom_point() +
    # geom_line(aes(group=patient)) +
    theme_minimal()
  
  ggplot(pd.need1,aes(x=PC1,y=PC2,color=`TNFa/NFkB`,shape=time_point)) + 
    geom_point() +
    # geom_line(aes(group=patient)) +
    theme_minimal()
  
  ggplot(pd.need1,aes(x=PC1,y=PC2,color=`APM/TC`,shape=time_point)) + 
    geom_point() +
    # geom_line(aes(group=patient)) +
    theme_minimal()
}
...
pdf(file=file.path(fig_dir1,"spp_cors_pre.pdf"),width=7,height=7)
heatmap3(pre,
         scale="none",balanceColor=T,main="Pre",margins=c(10,10))
dev.off()
write.csv(round(pre,3),file=file.path(fig_dir1,"spp_cors_pre.csv"))

##
changes <- cor(pd.need2 %>% filter(time_point == "Post") %>% dplyr::select(20:23),method="sp")
pdf(file=file.path(fig_dir1,"spp_cors_change.pdf"),width=7,height=7)
heatmap3(changes,
                  scale="none",balanceColor=T,main="change",margins=c(10,10))
dev.off()
write.csv(round(changes,3),file=file.path(fig_dir1,"spp_cors_change.csv"))
...
## 
pdf(file=file.path(fig_dir1,"spp_cors_pre_post.pdf"),width=7,height=7)
...
dev.off()
write.csv(round(pre.post,3),file=file.path(fig_dir1,"spp_cors_pre_post.csv"))
...
par(mar=c(5,10,4,1))
barplot(nlps,las=1,horiz=T,xlab="-log10(p-value)",main="significance of the coefficients")
...
```{r}
markers.wd <- readRDS(file=file.path(obj_dir,"15_dfbcc_snseq","markers_all_wehi_dfci_102824.rds"))
...
sapply(lst.wd,function(x)length(intersect(x,unlist(mods))))
```
...
```{r}
pdf(file=file.path(fig_dir1,"spp_nfkb_endo_pre_v2.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
plot(pd.need3pre$`TNFa/NFkB`,pd.need3pre$Endo,
     xlab="TNFa/NFkB",ylab="Endo",pch=20,col="grey60",
     main=paste0("correlation between TNFa/NFkB and Endo\n(spearman=",signif(sp.te,3),")"))
dev.off()
##
pdf(file=file.path(fig_dir1,"spp_dnfkb_dendo_v2.pdf"),width=7,height=7)
par(mar=c(5,5,3,3))
plot((pd.need3 %>% filter(time_point=="Pre"))$`d.TNFa/NFkB`,d.scores[,"d.Endo"],
     xlab="d.TNFa/NFkB",ylab="d.Endo",pch=20,col="grey60",
     main=paste0("correlation between d.TNFa/NFkB and d.Endo\n(spearman=",signif(d.sp.te,3),")"))
dev.off()
...
clin.ids <- tapply(pd.need.pre$id,pd.need.pre$my_response2,identity)
...
#### Plot
```{r}
pdf(file=file.path(fig_dir1,"ACSOG_volcano_baseline_all_smpls_v3.pdf"),width=7,height=7)
...
p <- ggplot(fg2.bl,aes(x=lab,y=slp)) + 
  geom_violin() +
  geom_point(data=fg2.bl %>% filter(pathway %in% names(mods)),aes(y=slp,color=pathway),pch=95,size=5) +
  geom_text_repel(data=fg2.bl %>% filter(pathway %in% names(mods)),aes(label=text),size=3) +
  scale_color_manual(values=mycols) +
  theme_minimal() +
  theme(legend.position="none")

pdf(file=file.path(fig_dir1,"baseline_fgsea_violin.pdf"),width=4,height=7)
print(p)
dev.off()
...
if(0){
  save(full_results,
       ...
       full_results_res,
       full_results_sens, 
       file=file.path(obj_dir,"clinical","ACOSOG_data.rda"))
}else{
  x <-  load(file=file.path(obj_dir,"clinical","ACOSOG_data.rda"))
}
```
