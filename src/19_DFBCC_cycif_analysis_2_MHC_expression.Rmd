...
### Load more objects
```{r}
df.cn.t <- readRDS(file.path(obj_dir1,"cn","CN_analysis_data.rds")) %>%
...
rowside <- cbind(`TIL-associated`=c("grey80","black")[(colnames(df.cn4) %in% c("CD8T","CD4T_nonTreg","CD4T_Treg","Mac_CD163","Mac_CD68_CD163"))+1])
...
```{r}
lst.mats <- lapply(used.abs[1:3],function(ab){
  mat <- mats[[ab]]
  cts1 <- lst.cst.def[[ab]]
...
pe.smpl <- cor(mats1,method="pe",use="pairwise.complete.obs")

cscs <- rep(csc[1:3],sapply(lst.cst.def[1:3],length))
pdf(file.path(fig_dir1,"exp_per_smpl","cor_pe_mean_exp_smpl.pdf"),width=7,height=7)
h3(pe.smpl,scale="none",main="PCC",balanceColor=TRUE,margins=c(10,10),
...
p.er1 <- ggplot(m.era,aes(x=d1,y=expression,color=nT)) +
  ...
  geom_line(data=df.cn3.seg,aes(x=x1,y=y1,group=i),color=1,linewidth=2) + 
  theme_minimal() +
...
pdf(file.path(fig_dir1,"exp_ts_axis","HLADPB1 expression per distance.pdf"),width=5,height=5)
print(p.dpb1)
dev.off()
```
...
pdf(file.path(fig_dir1,"exp_ts_axis","HLADPB1 vs nT.pdf"),width=5,height=5)
print(p.hla2)
dev.off()
```
...
```{r}
er.dep <- c(paste0("HLAABC_",c("Cancer","Endo","Mac_CD68","Mac_CD163","Mac_CD68_CD163","NK","B")),
...
```{r}
cts2 <- c("ER_indep","ER_dep")[(used.cts %in% sub("HLAABC_","",grep("HLAABC",er.dep,value=T)))+1]
names(cts2) <- used.cts
```
...
df.cn.hla1.dep <- df.cn.hla1 %>% 
  ...
  mutate(cell_types = factor(cell_types,levels=c("Cancer","Endo","B","NK","Mac_CD68","Mac_CD163","Mac_CD68_CD163"))) 
...
df.cn.hla1.ind <- df.cn.hla1 %>% 
  ...
  mutate(cell_types = factor(cell_types,levels=c("Fibro","CD8T","CD4T_nonTreg","CD4T_Treg","T_other")))
...
##
uniq.cols12 <- c("brown","grey80")
names(uniq.cols12) <- c("ER_dep","ER_indep")
cols12 <- uniq.cols12[cts2]
names(cols12) <- names(cts2)

d.exp12 <- c(d.exp2,d.exp1)
cols12.cts <- cols12[names(d.exp12)]
...
barplot(d.exp12,las=2,
        ...
        col=cols12.cts)
legend("topright",fill=uniq.cols12,legend=names(uniq.cols12))
dev.off()
```
...
pr.n <- c(1,12,3,4)[1:3]
par.types <- cbind(protein=rep(c(brewer.pal(7,"Set1")[c(2,1,3,4)])[1:3],c(pr.n)),
...
p.val <- t.test(hla ~ is.erdep,alternative="greater")$p.value # 7.9e-5
...
```{r}
used.abs <- c("ERa","HLAABC","HLADPB1","PD1")[1:3]
df.cn5 <- df.cn.t %>%
