...
pdf(file.path(fig_dir1,"mod_activity","esr1_heatmap.pdf"),width=7,height=7)
...
if(0){
  ...
  saveRDS(exp.gn.by.ct,file.path(obj_dir1,"mod_exp_by_ct.rds"))
}else{
  exp.gn.by.ct <-readRDS(file.path(obj_dir1,"mod_exp_by_ct.rds"))  
}
...
### expression of PD1/PDL1 in DC
```{r}
  cts1 <- wp$celltype_2
  is.dc <- cts1 == "Dendritic"

  mat.dc <- mat[,is.dc]
  smpls.dc <- factor(smpls[is.dc],levels=(df.pts %>% arrange(desc(ERS)) %>% pull(old)))

  dc.exp.by.smpl <- t(sapply(c("CD274","PDCD1"),function(gn){
    x <- mat.dc[gn,]
    tmp <- tapply(x,smpls.dc,mean,na.rm=T)
    # tmp <- tmp/max(tmp,na.rm=T)
    return(tmp)
  })) # 253 x 34 (pts)
  
  rownames(exp.gn.by.smpl) <- mod.gns
  cols <- brewer.pal(3,"Set1")[as.numeric(df.pts$subtype)]
  plot(tmp,main="PDL1 in dendritic cells",ylab="PD-L1 expression",xlab="low ERS <- patient index -> high ERS",col=cols,pch=19)
  legend("topright",pch=19,c("HR+","HER2+","TNBC"),col=brewer.pal(3,"Set1"))
```
...
#### per sample - done after reordering samples based on ERS scores
```{r}
...
xh <- h3(exp.gn.by.smpl[h.ri,o.smpls],scale="none",
     RowSideColors=mod.side.cols[h.ri,],ColSideColors=st.cols[o.smpls],
